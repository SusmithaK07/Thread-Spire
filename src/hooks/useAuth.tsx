...
interface AuthContextType {
  ...
  isLoading: boolean;
  login: (email: string, password: string, remember?: boolean) => Promise<{user: User | null, session: Session | null} | null>;
...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      return data; // Return the session and user data
    } catch (error) {
      toast({
  ...
  const logout = async () => {
    try {
      ...
      // We don't use useNavigate here to avoid Router context issues
    } catch (error) {
      toast({
